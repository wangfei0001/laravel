 ...
 */
class Core_PinAdapter extends Intems_Pattern_Singleton
{
    ...
    protected function _request($method, $data)
    {
        ...
        //get service url
        $url = PIN_SERVICE_URL .'webservice?t='.microtime(true).'&m='.$method;
        ...
        if(empty($result)){
             throw new Exception('No result from service[' .$url .']');
        }
        curl_close($con);
        ...
        if(empty($json)){
            throw new Exception('Request Error (Result is not Json):' .var_export($result, true));
        }
        return $json['result'];
    }
